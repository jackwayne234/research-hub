...
where $\eta(s)$ is the Dirichlet eta function (alternating zeta function). The bosonic series $\zeta(s) = \sum n^{-s}$ has all positive terms; the fermionic series $\eta(s) = \sum (-1)^{n+1} n^{-s}$ alternates in sign. The bridge factor:
...
$$\ln 2 = -\ln(1 - 2^{-1}) = -\ln\!\left(\frac{1}{2}\right)$$

which is the logarithm of the $p = 2$, $s = 1$ contribution to the Euler product. The $p = 2$ Euler factor $(1 - 2^{-s})^{-1}$ evaluated at $s \to 1$ gives the divergent harmonic series, but the logarithmic contribution of the single factor is $\ln(1/(1 - 1/2)) = \ln 2$.
...
$\delta_B$ increases 189% from no-detector to full-detector. The transition is continuous — no discontinuity at any coupling strength. All correlations are in the predicted direction: $r(\delta_B, D) = +0.36$, $r(\delta_B, \mathcal{V}) = -0.27$, $r(\delta_B, \alpha) = +0.25$.

### 8.2 Kretschner Scalar — Radial $\delta_B$ Profile
...
The key finding: $\delta_B$ remains below the bridge threshold $\varepsilon_B$ at every radius except $r \approx 1.001 \, r_s$ — the near-horizon crossing point. This means the Benford diagnostic validates the metric at every radius, certifying that curvature probes (Kretschner scalar, Ricci decomposition, Weyl tensor) are meaningful throughout the interior.
...
**Table 4.** Kretschner scalar $K$ at $r = 10^{-4} \, r_s$ for resolving vs. non-resolving models.
...
Shannon entropy is the logarithmic distance from order. It was always measuring $\delta_B$ in a different coordinate system.
...
### 10.2 $p = 3$ and Beyond: Deeper Structure

Higher primes generate deeper structural complexity in the integer structure:

| Prime | Partition | Physical examples |
|-------|-----------|------------------|
| $p = 2$ | Binary (even/odd) | Double slit, boson/fermion, spin-$\frac{1}{2}$ |
| $p = 3$ | Ternary | Triple slit, QCD color charge ($r, g, b$), three generations |
| $p = 5$ | Quinary | Higher-order interference, five-fold symmetries |
| All $p$ | Complete integer structure | Full physical reality |

The ternary case ($p = 3$) has concrete physical content. QCD color charge — the three-valued quantum number carried by quarks — requires a three-way partition that cannot be reduced to binary. In the prime framework, this is the $p = 3$ Euler factor $(1 - 3^{-s})^{-1}$ contributing to the metric. A triple-slit experiment, similarly, probes three-way coherence that is irreducible to pairwise ($p = 2$) interference. The higher-order Sorkin parameter $\kappa_3$, which measures genuine three-way interference beyond the sum of pairwise terms, is the $p = 3$ analogue of the $p = 2$ bridge factor.
...
8. **Higher primes generate deeper structure.** Binary distinctions ($p = 2$) are the simplest order. QCD color charge ($p = 3$), higher-order interference, and the full complexity of physical reality involve all primes.
